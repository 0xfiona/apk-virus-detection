...
#include <fcntl.h>
#include <string.h>
int is_apk(const char *file_name)
{
    int length;
    length=strlen(file_name);
    if (file_name[length-4] == '.' && file_name[length-3] == 'a' && file_name[length-2] == 'p' && file_name[length-1] == 'k') 
        return 1;
    return 0;
}
...
int detect(const char *file_name, const char *file_namebase){
	char hhhhh[1000] = "python3 /home/group9/test.py ";
    strcat(hhhhh, file_name);
    strcat(hhhhh, " ");
    strcat(hhhhh, file_namebase);
    
    
    char popenret[100000] = {0};/*执行脚本后的返回值*/
 
    memset(popenret, 0, sizeof(popenret));
    if(exec_process(hhhhh, popenret, sizeof(popenret)) != 0) {
        printf("RUN %s faild", hhhhh);
        return -1;
    }
    // printf("%s \n", popenret);
	 int length;
    length=strlen(popenret);
    if (popenret[length-1] == '0') return 1;
    else return 0;
}
...
cl_error_t cl_scandesc_callback(int desc, const char *filename, const char **virname, unsigned long int *scanned, const struct cl_engine *engine, struct cl_scan_options *scanoptions, void *context)
{
    ...
    if (is_apk(filename)){
        ...
        temp = detect(filename,filename_base);
        if (temp == 1){ 
				status = CL_VIRUS;
				engine->cb_virus_found(desc, "Group9 detected virus", context);
		}
        else status = CL_CLEAN;
    }
    else{
